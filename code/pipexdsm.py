    ...
    for i, location in enumerate(transect_names):
        ...
        with ProcessPoolExecutor(num_workers) as pex:
            for laz in loc_laz_files:
                ...
                pex.submit(pointcloud_to_dsmtiff, laz,tif,lazo,res,outfn,method,lc,hc,mode2)

    tf = time.perf_counter() - ti 
    print(f"run.time = {tf/60} min(s)")

# add to the code so that for every batch print x of total files being process
# batch comes from num_workers in parallel processing and total is the totla of all laz files 
